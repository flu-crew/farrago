...
for file in "${filelist[@]}"; do
    ...
    echo "$tempdir/${file}_sequences"
    sed -n "/>/p" $file | sed -e "s/^>//g" | sort | uniq > "$tempdir/${file}_sequences"
done

cat "$tempdir/$1_sequences" > "$tempdir/common_sequences"
echo "Finding subset"
for file in "${filelist[@]}"; do
    comm -12 "$tempdir/common_sequences" "$tempdir/${file}_sequences" | sort > "$tempdir/temporary_file"
